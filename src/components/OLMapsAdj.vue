<template>
  ...
  <div class="options">
    显示/隐藏：
    <input type="checkbox" checked="checked" @click="checkTiandi($event)" />天地图 <input type="checkbox" checked="checked" @click="checkBaidu($event)" />百度
    <input type="checkbox" checked="checked" @click="checkAmap($event)" />高德地图 <input type="checkbox" checked="checked" @click="checkOMS($event)" />OMS地图
    <input type="checkbox" checked="checked" @click="checkPoint($event)" />点 <input type="checkbox" checked="checked" @click="checkVector($event)" />矢量图
  </div>
  <div class="options">
    图层顺序：
    <input name="seq" type="radio" value="" @click="upTiandi($event)" />天地图最上 <input name="seq" type="radio" value="" checked="checked" @click="upBaidu($event)" />百度最上
    <input name="seq" type="radio" value="" checked="checked" @click="upAmap($event)" />高德地图
...
import { Style, Text, Icon, Fill, Stroke, Image as ImageStyle, Circle as CircleStyle } from "ol/style";
...
export default {
  ...
  methods: {
    ...
    addBmap() {
      ...
      var source = new TileImage({
        ...
        tileUrlFunction: function (tileCoord, pixelRatio, proj) {
          ...
          return `https://maponline0.bdimg.com/tile/?qt=vtile&x=${x}&y=${y}&z=${z}&styles=pl&scaler=1&udt=20220819&from=jsapi2_0`;
        },
      });
    ...
    initMap() {
      ...
      // 添加百度地图
      this.baiduLayer = this.addBmap();
      // this.baiduLayer = addBaiduLayer()
      // 添加高德地图
    ...
    addPoint() {
      ...
      var activity = new Feature({
        geometry: new Point(
          ...
          // transform([108.9420872926712, 34.224372804164894], "EPSG:4326", "GCJ-02")
          transform([108.9420872926712, 34.224372804164894], "EPSG:4326", "EPSG:3857")
